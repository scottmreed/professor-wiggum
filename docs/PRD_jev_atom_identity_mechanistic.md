...
| Eval tiers easy/medium/hard exist for acceptance tests | `training_data/eval_tiers.json` has easy=100, medium=0, hard=0. Multistep cases live in `flower_mechanisms_multistep.json` (120) and the holdout (98 cases, 451 steps). | Acceptance criteria that cite medium/hard need those tiers populated first (§19 Phase 0). |
...
- Populate `eval_tiers.json` medium and hard from `flower_mechanisms_multistep.json` / holdout, or document which policy file supplies those tiers, so acceptance criteria below are runnable.
