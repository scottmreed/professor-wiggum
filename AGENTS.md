...
- `inference_call_started` / `inference_call_completed` / `inference_call_failed` are emitted live per chat-model request (`mechanistic_agent/core/call_recorder.py`): `_mark_step_started` opens a per-thread call context and `llm.get_chat_model` returns a recording proxy while it is open, so latency is measured and a `tools.py` fallback-model retry appears as a second call with `fallback_from_call_id`. Outside a run step `get_chat_model` returns the bare adapter. Jev calls are derived from `output.decision_trace` (`DecisionRecord`); a swallowed provider error (`output.status` in `failed|fallback` with `output.error`) on a step with no live record is a failed call. `_record_step` never emits a call twice.
- Step cost (`_extract_step_cost`) is priced against `output.model_used` when the tool reports it, so a provider fallback is billed to the model that answered.
...
- `mechanism_step_accepted.atom_identity` (`atom_identity.v1`) carries the mapped loop state after the step (persistent `pid`, `map_number`, `element` per atom, `mapped_species`, preserved/new/lost counters, `identity_source` = `persistent` only under `loop_state_mapping: mapped`, else `derived`); `/observatory` projects it into `atom_lineage` and the UI shows an Atom lineage table.
