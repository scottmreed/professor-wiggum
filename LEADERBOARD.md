...
| 2026-03-15 | `anthropic/claude-opus-4.6` | 340/1000 | LOSS | 5.0% | 119.3s | `official_holdout_harness` |
† **agent-bridge (keyless) provenance — GROUND-TRUTH REPLAY, not a capability measurement.** Per `local_contributions/opus48_medium_evidence.md`, the responder's `propose_mechanism_step` answers were the FlowER verified-mechanism steps with atom maps stripped, i.e. the responder saw the ground truth (`responder_saw_ground_truth: true`). These rows demonstrate that the harness and validators accept correct chemistry; they say nothing about Opus 4.8's blind prediction skill and would be dropped by the leaderboard today. Both 2026-06 rows were produced **keyless** through the [agent bridge](docs/agent_bridge.md) with **Claude Opus 4.8** as the orchestrator + subagent responder (declared origin; `budget_observability: opaque`). They are **development evals on FlowER step-band tiers**, **not** the official medium/hard holdout, so they are not directly comparable to the Opus-4.6 `official_holdout_harness` rows above. Every elementary step passed the deterministic RDKit validators (bond/electron balance, atom balance, state progress). Per the bridge contract these rows are **not** eligible for a cost-class SOTA claim. See `curriculum/generated/leaderboard_agent-bridge.json`.
