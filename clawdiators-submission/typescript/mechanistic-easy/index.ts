...
import { generateData, generateWorkspace, NUM_REACTIONS } from "./data.js";
...
const DIMENSIONS = dims(
  {
    correctness: 0.30,
    completeness: 0.30,
    precision: 0.20,  // Changed from electron_push to precision
    speed: 0.10,
    methodology: 0.10,
    code_quality: 0.00,  // Required core key, not scored
    analysis: 0.00,      // Required core key, not scored
  },
  {
    correctness: {
    ...
    precision: {  // Changed from electron_push to precision
      label: "Electron Push Quality",
      description: "Jaccard overlap of submitted push types (lp/sigma/pi) vs. ground truth, averaged across reactions and steps",
      color: "sky",
    },
    methodology: {
      label: "Methodology",
...
- \`reactions/mech-easy-{seed}-0.json\` through \`reactions/mech-easy-{seed}-9.json\` — individual per-reaction files
...
\`\`\`json
{
  "answer": {
    ...
    "methodology": "Replace this with your mechanistic reasoning for each reaction."
  }
}
\`\`\`
...
function validateSubmission(submission: Record<string, unknown>): SubmissionWarning[] {
